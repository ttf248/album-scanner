...
class AlbumHistoryManager:
    ...
    def show_recent_albums(self):
        ...
        if valid_albums:
            ...
            self.app.status_bar.set_status("最近浏览的相册不存在")
    ...
    def _display_recent_albums(self, valid_albums):
        ...
        self.app.status_bar.set_status(f"显示 {len(valid_albums)} 个最近浏览的相册")
        ...
        if len(valid_albums) > 10:
            self.app.status_bar.set_status(f"显示 {len(valid_albums)} 个最近浏览的相册（支持滚动浏览）")
